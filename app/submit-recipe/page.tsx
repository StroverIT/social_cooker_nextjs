...
import { Recipe, Ingredient, CATEGORIES, TAGS, DIET_TYPES } from "@/types";
...
export default function RecipeSubmissionPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (ingredients.some((ing) => !ing.name.trim() || ing.amount <= 0)) {
      toast({
        title: "Грешка",
        description: "Моля, попълнете всички съставки коректно",
        variant: "destructive",
      });
      return;
    }
    if (instructions.some((inst) => !inst.trim())) {
      toast({
        title: "Грешка",
        description: "Моля, попълнете всички стъпки за приготвяне",
        variant: "destructive",
      });
      return;
    }
    ...
    const newRecipe: Recipe = {
      ...
      tags: selectedTags,
      dietTypes:
        selectedDietTypes.length > 0 ? selectedDietTypes : ["balanced"],
    ...
    toast({
      ...
      description:
        "Вашата рецепта ще бъде прегледана от администратор преди публикуване.",
    });
    router.push("/");
  };
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-6">
        <div className="space-y-4">
          ...
          <div>
            ...
            <Select
              value={category}
              onValueChange={(v: Recipe["category"]) => setCategory(v)}
            >
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {DIET_TYPES.map((diet) => (
              <Button
                ...
                type="button"
                variant={
                  selectedDietTypes.includes(diet.id) ? "default" : "outline"
                }
                size="sm"
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <Label htmlFor="protein" className="text-macro-protein text-xs">
                Протеини (г)
              </Label>
              <Input
                id="protein"
            ...
            <div>
              <Label htmlFor="carbs" className="text-macro-carbs text-xs">
                Въглехидрати (г)
              </Label>
              <Input
                id="carbs"
            ...
            <div>
              <Label htmlFor="fat" className="text-macro-fat text-xs">
                Мазнини (г)
              </Label>
              <Input
                id="fat"
        ...
        <p className="text-xs text-muted-foreground text-center">
          Рецептата ще бъде прегледана от администратор преди да бъде
          публикувана.
        </p>
      </form>
